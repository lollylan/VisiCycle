...
def auto_backup_worker():
    """Background worker that creates an encrypted backup every day at 23:00."""
    while True:
        now = datetime.now()
        # Check if it's 23:00
        if now.hour == 23 and now.minute == 0:
            try:
                # Ensure we are unlocked so we can encrypt
                if encryption_service.is_unlocked:
                    db_path = "db.sqlite3"
                    if os.path.exists(db_path):
                        # Read the raw db file
                        with open(db_path, "rb") as f:
                            raw_data = f.read()

                        # Encrypt the database file bytes
                        encrypted_data = encryption_service.encrypt_bytes(raw_data)

                        # Determine backup filename and folder
                        backup_dir = "backups"
                        os.makedirs(backup_dir, exist_ok=True)
                        timestamp = now.strftime("%Y-%m-%d_%H-%M-%S")
                        backup_filename = f"visicycle_backup_{timestamp}.sqlite3.enc"
                        ...
                        print(f"Auto-backup successful: {backup_path}")
                else:
                    print("Auto-backup skipped: App is locked.")
            except Exception as e:
                print(f"Error during auto-backup: {e}")
            
            # Sleep a bit longer than a minute so we don't trigger twice in the same minute
            time.sleep(65)
        else:
...
def export_database(_=Depends(require_unlocked)):
    """Downloads the current encrypted SQLITE database file."""
    db_path = "db.sqlite3"
    ...
    filename = f"visicycle_backup_{timestamp}.sqlite3.enc"
...
async def import_database(file: UploadFile = File(...), _=Depends(require_unlocked)):
    ...
    try:
        ...
        # Check if it's already a plain SQLite file
        if raw_data.startswith(b"SQLite format 3"):
            decrypted_data = raw_data
        else:
            # Try to decrypt it
            try:
                decrypted_data = encryption_service.decrypt_bytes(raw_data)
            except Exception as e:
                raise HTTPException(status_code=400, detail="Das Backup konnte nicht entschlüsselt werden. Falsches Passwort oder beschädigte Datei.")
                
        # Validate that we possess a valid SQLite header
        if not decrypted_data.startswith(b"SQLite format 3"):
            raise HTTPException(status_code=400, detail="Ungültige oder beschädigte Datenbankdatei.")
